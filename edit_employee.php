...
// Handle update (keep same functionality / fields)
...
<head>
    ...
    <title><?php echo $company ? htmlspecialchars($company['company_name']) : 'Employee Management System'; ?> - Edit Employee</title>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .user-profile { display:flex; align-items:center; gap:10px; padding:8px 15px; border-radius:25px; background:rgba(255,255,255,.1); transition:.3s; }
        ...
        .sidebar-menu a {
            color:#bdc3c7; padding:12px 15px; display:flex; align-items:center; text-decoration:none;
            border-radius:10px; transition:.3s; font-weight:500; gap:12px;
        }
        .sidebar-menu a:hover, .sidebar-menu a.active {
            background:linear-gradient(135deg,#3498db,#2980b9); color:#fff; transform:translateX(5px);
            box-shadow:0 4px 15px rgba(52,152,219,.3);
        }
        .sidebar-menu a i { width:20px; font-size:16px; }

        /* Content Area */
        .content { margin-left:280px; margin-top:70px; padding:30px; transition:margin-left .3s ease; min-height:calc(100vh - 70px); }
        ...
        .form-header { text-align:center; margin-bottom:40px; padding-bottom:20px; border-bottom:2px solid #f8f9fa; }
        .form-title { font-size:2rem; font-weight:700; color:#2c3e50; margin-bottom:10px; display:flex; align-items:center; justify-content:center; gap:15px; }
        .form-subtitle { color:#7f8c8d; font-size:1.1rem; font-weight:400; }

        /* Controls */
        .form-group { margin-bottom:25px; }
        .form-label { font-weight:600; color:#2c3e50; margin-bottom:8px; display:block; font-size:.95rem; }
        .form-control,.form-select {
            border:2px solid #e9ecef; border-radius:12px; padding:12px 16px; font-size:1rem; transition:.3s;
        ...
        .bank-fields-section {
            ...
            border-radius:15px; padding:25px; margin-top:20px; border:2px dashed #dee2e6; transition:.3s;
        }
        .bank-fields-section.show { border-color:#667eea; background:linear-gradient(135deg,#f0f4ff 0%,#e6f2ff 100%); }

        /* Buttons */
        .btn-primary {
            background:linear-gradient(135deg,#667eea 0%,#764ba2 100%); border:none; padding:14px 30px;
            font-weight:600; border-radius:12px; font-size:1.1rem; transition:.3s; box-shadow:0 4px 15px rgba(102,126,234,.3);
        }
        .btn-primary:hover { transform:translateY(-2px); box-shadow:0 8px 25px rgba(102,126,234,.4); background:linear-gradient(135deg,#5a67d8 0%,#6b46c1 100%); }
        .btn-secondary {
            background:linear-gradient(135deg,#95a5a6 0%,#7f8c8d 100%); border:none; padding:14px 30px; font-weight:600;
            border-radius:12px; font-size:1.1rem; transition:.3s; color:#fff;
        }
        .btn-secondary:hover { transform:translateY(-2px); background:linear-gradient(135deg,#7f8c8d 0%,#6c7b7d 100%); color:#fff; }

        /* Alerts & Animations */
        .alert-danger { background:linear-gradient(135deg,#ff6b6b 0%,#ee5a52 100%); border:none; border-radius:12px; color:#fff; padding:15px 20px; font-weight:500; }
        ...
        /* Input Icons */
        .input-group-text {
            background:linear-gradient(135deg,#667eea 0%,#764ba2 100%); border:none; color:#fff; border-radius:12px 0 0 12px;
        }
...
<body>
            ...
            <div class="col-xl-10 col-lg-11">
                <div class="form-card fade-in">
                    <div class="form-header">
                        ...
                        <p class="form-subtitle">Update the details and save changes</p>
                    </div>
                    ...
                    <form method="POST" id="employeeForm">
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <select id="payment_type" name="payment_type" class="form-select" required>
                                        <option value="cash" <?php echo ($employee['payment_type'] === 'cash') ? 'selected' : ''; ?>>Cash Payment</option>
                                        <option value="bank" <?php echo ($employee['payment_type'] === 'bank') ? 'selected' : ''; ?>>Bank Transfer</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <?php $isBank = ($employee['payment_type'] === 'bank'); ?>
                        <div id="bank_fields"
                             class="bank-fields-section <?php echo $isBank ? 'show' : ''; ?>"
                             style="display: <?php echo $isBank ? 'block' : 'none'; ?>;">
                            ...
                            <div class="row">
                                <div class="col-md-6 form-group">
                                    ...
                                    <select id="bank_name" name="bank_name" class="form-select">
                                        ...
                                        <option value="Other" <?php echo ($employee['bank_name'] === 'Other') ? 'selected' : ''; ?>>Other Bank</option>
                                        <option value="Apprentice" <?php echo ($employee['bank_name'] === 'Apprentice') ? 'selected' : ''; ?>>Apprentice Account</option>
                                    </select>
                                </div>
                                <div class="col-md-6 form-group">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 form-group">
                                    ...
                                    <input type="text" id="micr_code" name="micr_code"
                                           ...
                                           value="<?php echo htmlspecialchars($employee['micr_code']); ?>"
                            ...
                            <div class="row">
                                <div class="col-md-6 form-group">
                                    ...
                                    <input type="text" id="provident_fund_ac_no" name="provident_fund_ac_no"
                                           ...
                                           value="<?php echo htmlspecialchars($employee['provident_fund_ac_no']); ?>"
                                ...
                                <div class="col-md-6 form-group">
                                    ...
                                    <input type="text" id="uan_no" name="uan_no"
                                           class="form-control" maxlength="50"
                                           value="<?php echo htmlspecialchars($employee['uan_no']); ?>"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Payment Type Toggle (same behavior as Add page)
            ...
            function toggleBankFields() {
                if (paymentType.value === 'bank') {
                    ...
                    requiredInputs.forEach(inp => inp.required = true);
                    bankNameSelect.required = true;
                } else {
                    bankFields.style.display = 'none';
                    bankFields.classList.remove('show');
                    requiredInputs.forEach(inp => inp.required = false);
            ...
            // Basic form validation (mirrors Add page)
